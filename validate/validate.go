...
func exactByteMatch(anyQuote any, opts *Options) error {
	...
	// V5 TDX 1.0 and V4 quotes do not have the fields MinimumTeeTcbSvn2 and MrServiceTd,
	// so we don't check them for those quotes.
...
func TdxQuote(quote any, options *Options) error {
	...
	return multierr.Combine(
		...
		validateTdAttributes(tdAttributes, tdAttributesFixed1, tdAttributesFixed0, options.TdQuoteBodyOptions.EnableTdDebugCheck, options.TdQuoteBodyOptions.EnableTdMigratableCheck),
	)
}
