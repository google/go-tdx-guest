...
func (r *recordingGetter) Get(url string) (map[string][]string, []byte, error) {
	r.getCalls++
...
func (r *recordingContextGetter) GetContext(ctx context.Context, url string) (map[string][]string, []byte, error) {
	r.getContextCalls++
	select {
	case <-ctx.Done():
